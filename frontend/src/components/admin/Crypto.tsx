...
export default function Crypto() {
  ...
  const load = () => {
    ...
    api.get('/admin/config').then((r) => { setCoins(r.data.config.cryptoCoins ?? []); setCryptoEnabled(r.data.config.cryptoEnabled ?? true); }).catch(() => {});
  };
  useEffect(() => { load(); }, []);
  ...
  const saveCoins = async () => { try { await api.patch('/admin/config', { cryptoEnabled, cryptoCoins: coins }); setMsg('✓ Crypto settings saved'); } catch { setMsg('Could not save'); } };
    ...
    <div className="space-y-5">
      ...
      <div className="glass space-y-3 p-4">
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-[1fr_2fr_auto] items-center gap-2 text-[11px] uppercase tracking-wider text-white/40"><span>Symbol</span><span>Rate (₹ per coin)</span><span>On</span></div>
          {coins.map((c) => (
            <div key={c.symbol} className="grid grid-cols-[1fr_2fr_auto] items-center gap-2">
